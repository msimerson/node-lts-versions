...
class getNodeLTS {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.nodeVersionData().then(versions => {
        ...
        for (const [maj, obj] of Object.entries(this.majorsInitial)) {
          this.majorsLatest[maj].dateEndCurrent = this.deltaDate(obj.date, [0,6,0])
          if (maj % 2 === 0) {
            this.majorsLatest[maj].dateStartLTS     = this.deltaDate(obj.date, [0,6,0])
            this.majorsLatest[maj].dateEndActiveLTS = this.deltaDate(obj.date, [0,12,0])
            this.majorsLatest[maj].dateEndLTS       = this.deltaDate(obj.date, [0,36,0])
  ...
  get (filter) {
    ...
    case 'maintenance':
      fn = ([maj, obj]) => {
        return obj.lts && obj.dateEndActiveLTS.getTime() < now && obj.dateEndLTS.getTime() > now
      }
      break;
    case 'lts':
      fn = ([maj, obj]) => {
  ...
  _printLatest() {
    ...
    for (const m of this.get('lts')) {
      ...
      console.log(util.format(`%s    %s\t%s on %s\t%s to %s`, m, v.lts, v.version, v.date, v.dateStartLTS.toISOString().slice(0,10), v.dateEndLTS.toISOString().slice(0,10)))
    }
  }
  ...
  deltaDate (input, ymd = [0,6,0]) {
    ...
    return new Date(
      ...
      Math.min(
        input.getDate() + ymd[0],
        new Date(input.getFullYear() + ymd[2], input.getMonth() + ymd[1] + 1, 0).getDate()
      )
    );
  }
